...
Nothing here is one program. Three repos with three different scopes:
...
| [`sandbox-factory`](https://github.com/jeffjacobsen/sandbox-factory) | the factory: ADW workflows, the agent roster, the trace UI | stamped into any repo |
| [`inkwell`](https://github.com/jeffjacobsen/inkwell) | the app, its prompts, and a `sandbox.yaml` saying how to run it | one project |

The split matters for a practical reason: `sbx manage reap` and `sbx manage list`
reason about a whole exe.dev + OpenRouter **account**, so a copy of the
orchestrator per repo would give one partial view per repo and let a key hide in
the gap. It is installed once:
