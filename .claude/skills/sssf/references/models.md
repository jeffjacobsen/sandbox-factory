# Models: rosters, rates, the pi registry

What the ADWs run on: the two rosters in `adws/adw_sssf_config/`, the ten models registered in
`sandbox_mount/guest/models.json.tmpl`, and the four blocks each entry must carry. All of it lives
in THIS repo — it moved out of the sbx orchestrator, which mounts any repo and has no business
knowing our roster. sbx's side of the boundary (minting, revoking, the credential lifecycle) is in
its own `references/credentials.md`.
...
```bash
sbx run cmd <id> 'pi --list-models'    # any row reading 128K 16.4K no no is under-declared
```
...
## Zero data retention, as it lands on the roster

Enforcement is an **account** setting, not a per-request one — pi's `models.json` supports custom
headers but no arbitrary request-body fields, so `provider: {...}` cannot be injected through pi's
config at all. It is set once in the OpenRouter dashboard (Settings → Privacy) and then applies to
every request from every key. That half is the orchestrator's to explain; what matters here is
where it lands on the roster:

- Gate C — our own `gate_factory.sh` — sends `"provider":{"zdr":true,"data_collection":"deny"}` on
  every roster model, so the gate proves ZDR routing resolves before any real work runs.
- Under ZDR, `deepseek-v4-flash-0731` is served by **Parasail** and `glm-5.2` by **CoreWeave**, both
  US providers. The "Chinese labs may train on your data" caution applies to those labs' own APIs,
  not to their open weights on Western hosts. Under ZDR you are not talking to a Chinese lab at all.
